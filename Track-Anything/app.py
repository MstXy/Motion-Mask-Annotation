...
# ---------------------
## reverse
def reverseSeq(doReverse):
    doReverse["reverse"] = not doReverse["reverse"]
    print("Reverse: ", doReverse["reverse"])
    reverse_state_text = doReverse["reverse"]
    return doReverse, reverse_state_text
# ----------------------

# extract frames from upload video
def get_frames_from_video(video_input, video_state, doReverse):
    """
    Args:
        video_path:str
        timestamp:float64
    ...
    """
        video input is now a txt file indicating data name.
        txt name format: xx_2_xxxx-xxxx for data folder xx/image_2/xxxx.png - xx/image_2/xxxx.png 
        
    """
    video_path = video_input
    user_name = time.time()
    operation_log = [("",""),("Upload video already. Try click the image for adding targets to track and inpaint.","Normal")]
    fps = 10 # should be the frame rate for KITTI

    # Create dir
    file_name = video_input.name.split('/')[-1].split('.')[0]
    file_info = file_name.split("_")
    file_path = f'{args.kittidir}/odometry/dataset/sequences/{file_info[0]}/image_{file_info[1]}'
    # if os.path.isdir(file_path):
    #     os.system(f'rm -r {file_path}')
    # os.makedirs(file_path)
    # # Unzip file
    # os.system(f'unzip {video_input.name} -d ./assets ')
    
    _startframe = int(file_info[2].split('-')[0])
    _endframe = int(file_info[2].split('-')[1]) + 1
    imgs_path = sorted([os.path.join(file_path, img_name) for img_name in os.listdir(file_path) if int(img_name.split('.')[0]) in range(_startframe, _endframe)], reverse=doReverse["reverse"])
    ## check if width divisible by 2, if not, crop by 1 pixel
    sf = cv2.imread(imgs_path[0]) # sample frame
    sf_w = sf.shape[0]
    sf_h = sf.shape[1]
    ini_frames = [cv2.imread(path)[:-1 if sf_w % 2 else sf_w,:-1 if sf_h % 2 else sf_h] for path in imgs_path]
    frames = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in ini_frames]
    # -------------------------    
    ...
    video_state = {
        ...
        ## image sequence
        "video_name": file_name + ".mp4", # save to mp4
        "file_info": file_info, # for saving
        # -----------------------------
...
# tracking vos
def vos_tracking_video(video_state, interactive_state, mask_dropdown, doReverse):
    ...
    # -----change output dir-------
    video_output = generate_video_from_frames(video_state["painted_images"], output_path="{}/track/{}".format(interactive_state["save_dir"], video_state["video_name"]), fps=fps) # import video_input to name the output video
    ...
    if interactive_state["mask_save"]:
        # -----change output dir-------
        filepath = '{}/mask/{}/{}/{}'.format(
                        interactive_state["save_dir"], video_state["file_info"][0], video_state["file_info"][1], video_state["file_info"][2])
        if not os.path.exists(filepath):
            os.makedirs(filepath)
        # -----------
        ## check if reverse
        if doReverse["reverse"]:
            i = len(video_state["masks"]) - 1
        else:
            i = 0
        # -------------
        print("save mask")
        for mask in video_state["masks"]:
            # np.save(os.path.join('W/mask/{}'.format(video_state["video_name"].split('.')[0]), '{:05d}.npy'.format(i)), mask)
            # -------------------
            ## save as png
            ## change output dir
            Image.fromarray(mask).save(os.path.join(filepath, '{:08d}.png'.format(i)))
            ## check if reverse
            if doReverse["reverse"]:
                i -= 1
            else:
...
with gr.Blocks() as iface:
    ...
    with gr.Row():
        ...
        with gr.Column():
            with gr.Row(scale=0.4):
                ...
                ## for image sequence input
                # video_input = gr.inputs.File(file_count="directory")
                video_input = gr.File()
                # -------------------------
                with gr.Column():
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    with gr.Column():
                        ...
                        # ----------------------
                        ## add reverse sequence button
                        with gr.Row():
                            reverse_sequence = gr.Button(value="Reverse", interactive=True)
                            reverse_state_text = gr.Textbox(value="false", label="Reverse")
                        # ----------------
    ...
    # -------------------------      
    ## ini step: set if reverse
    reverse_sequence.click(
        fn=reverseSeq,
        inputs=doReverse,
        outputs=[doReverse, reverse_state_text]
    )
    # -------------------------
    ...
    extract_frames_button.click(
        ...
        inputs=[
            video_input, video_state, doReverse
        ],
    ...
    tracking_video_predict_button.click(
        ...
        inputs=[video_state, interactive_state, mask_dropdown, doReverse],
